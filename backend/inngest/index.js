...
export const inngest = new Inngest({ id: "hirearchy-ems" });
...
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{cron: "TZ=Asia/Manila 30 11 * * *"}]}, 
  
    async ({ step }) => {
      const today = await step.run("get-today-date", () =>{
        const startUTC = new Date(new Date().toLocaleDateString("en-CA", {timeZone: "Asia/Manila"}) + "T00:00:00+08:00");
      ...
      const activeEmployees = await step.run("get-active-employees", async () => {
        ...
        return employees.map((e) => ({_id: e._id.toString(), firstName: e.firstName, lastName: e.lastName, email: e.e.email, department: e.department}))
      })
