    ...
    <div className="animate-fade-in">
      ...
      {loading ? (
        <div className="flex justify-center p-12">
          <div className="animate-spin size-8 border-2 border-slate-600 border-t-transparent rounded-full"/>
        </div>
      ): (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
          {filtered.length === 0 ? (
            ...
            filtered.map((emp) => (<p key={emp.id}>{emp.firstName} {emp.lastName}</p>))
          )}
        </div>
      )}
    </div>
